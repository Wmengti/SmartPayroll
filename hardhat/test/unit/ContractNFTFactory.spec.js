 ...
 * @LastEditTime: 2023-07-13 15:28:43
 * @Description:
 */
...
describe("ContractNFTFactory Unit Tests", async function () {
  ...
  const deployFixture = async () => {
    ...
    const contractNFTclone = await (await ethers.getContractFactory("ContractNFTClone", owner)).deploy();
    await contractNFTclone.deployed();
    console.log(contractNFTclone.address);
    const contractNFTFactory = await (
      await ethers.getContractFactory("ContractNFTFactory", owner)
    ).deploy(contractNFTclone.address);
  ...
  describe("factory clone contract NFT", () => {
    it("NFT clone success", async () => {
      const { contractNFTFactory, owner, user2 } = await loadFixture(deployFixture);
      await expect(contractNFTFactory.connect(owner).cloneTask(user2.address, "solidity"))
        .to.be.emit(contractNFTFactory, "CreateContractNFT")
        .withArgs(anyValue, "solidity");
    });
  });
