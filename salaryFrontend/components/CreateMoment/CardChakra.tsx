 ...
 * @LastEditTime: 2023-06-12 17:38:07
 * @Description:
 */
import {
  Card,
...
const CardChakra = (props: any) => {
  ...
  useEffect(() => {
    ...
    const balanceHandler = async () => {
      let contractBalance = await erc20Token?.balanceOf(
        props.contract.upkeeperContract
      );
      ...
      vaultBalance = parseInt(
        utils.formatUnits(vaultBalance, tokenSymbol[props.contract.token])
      );
  ...
  const saveHandler = async () => {
    ...
    try {
      ...
      toast(
        "The contract has been locked.  transfer the remaining balance to the DAO vault. ",
        {
          position: "top-center",
          autoClose: 5000,
        }
      ...
      <Card maxW="sm" bg="gray.100">
        <CardBody>
          ...
          <Stack mt="6" spacing="3">
            ...
            <Text>
              Desposit {props.contract.amount * props.contract.round}{" "}
              {props.contract.token}
            </Text>
            <Text>
              Balance {contractBalance} {props.contract.token}
            </Text>
            <Text>
              Lock {vaultBalance} {props.contract.token}
            </Text>
          </Stack>
        </CardBody>
        <Divider />
          ...
          <ModalContent>
            ...
            <ModalBody pb={6}>
              <Stack spacing={2} px={4}>
                ...
                <Text fontSize="lg" bg="gray.100">
                  {props.contract.admin}
                </Text>
                <Text fontSize="lg" as="b">
                  Employee Address
                </Text>
                <Text fontSize="lg" bg="gray.100">
                  {props.contract.receiver}
                </Text>
                <Text fontSize="lg" as="b">
                  DAOVault Address
                </Text>
                <Text fontSize="lg" bg="gray.100">
                  {props.contract.DAOAddress}
                </Text>
                <Text fontSize="lg" as="b">
                  Work Type
                </Text>
                <Text fontSize="lg" bg="gray.100">
                  {props.contract.workType}
                </Text>
                <Text fontSize="lg" as="b">
                  Salary
                </Text>
                <Text fontSize="lg" bg="gray.100">
                  {props.contract.amount} {props.contract.token}
                </Text>
                <Text fontSize="lg" as="b">
                  Sending Cycle
                </Text>
                <Text fontSize="lg" bg="gray.100">
                  {props.contract.timeInterval} {props.contract.timeUint}
                </Text>
                <Text fontSize="lg" as="b">
                  Sending Round
                </Text>
                <Text fontSize="lg" bg="gray.100">
                  {props.contract.round}
                </Text>
                <Text fontSize="lg" as="b">
                  Contract State
                </Text>
